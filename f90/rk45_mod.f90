!!!@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
!!!@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
!!! THIS FILE CONTAINS PARTS OF THE "rk45" code as written Herman Watts, 
!!! Lawrence Shampine and adapted to F90 by John Burkardt (see comments below).
!!! 
!!! The rk45 code has been edited slighly by wrapping it in module form
!!! and  a module "precision_type.f90", so that the precision of 
!!! real-variables is set in a centralized manner symmetrical to the 
!!! rest of the ION-CAGE code. So instead of declaring reals as
!!! > real(kind=8) :: x 
!!! > real(kind=8) :: y
!!! etc., it is now
!!! > use precision_type (contains variable "dp" with desired precision)
!!! > real(dp) :: x 
!!! > real(dp) :: y
!! throughout.!
!!!@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
...
!
  ...
  real ( dp ) ch
  external f
  real ( dp ) f1(neqn) ! REPLACED ( KIND = 8 ) with ( dp ) and added "use precision type" in the top
  real ( dp ) f2(neqn)
  real ( dp ) f3(neqn)
  real ( dp ) f4(neqn)
  real ( dp ) f5(neqn)
  ...
  call f ( t + ch, f5, f1 ) ! JSV COMMENTED

  ch = 3.0D+00 * h / 32.0D+00

  f5(1:neqn) = y(1:neqn) + ch * ( yp(1:neqn) + 3.0D+00 * f1(1:neqn) )

  call f ( t + 3.0D+00 * h / 8.0D+00, f5, f2 )       ! JSV COMMENTED
  ...
  call f ( t + 12.0D+00 * h / 13.0D+00, f5, f3 )       ! JSV COMMENTED
  ...
  call f ( t + h, f5, f4 )       ! JSV COMMENTED
  ...
  call f ( t + h / 2.0D+00, f1, f5) ! JSV COMMENTED
!
...
!
  ...
  real ( dp ) abserr
  real ( dp ), save :: abserr_save = -1.0D+00
  real ( dp ) ae
  real ( dp ) dt
  real ( dp ) ee
  real ( dp ) eeoet
